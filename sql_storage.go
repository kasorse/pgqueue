...
func (s *sqlStorage) createTaskTx(ctx context.Context, tx sqlx.Tx, kind int16, maxAttempts uint16, payload []byte, ttlSeconds uint32) error {
	insertQuery := `
		INSERT INTO public.queue (kind, attempts_left, payload, expires_at)
		VALUES ($1, $2, $3, $4)
	`

	expiresAt := time.Now().Add(time.Duration(ttlSeconds) * time.Second)

	_, err := tx.ExecContext(ctx, insertQuery, kind, maxAttempts, string(payload), expiresAt)
	return err
}
...
func (s *sqlStorage) getTasks(ctx context.Context, kind int16, workerCountLimitForInstance uint16, workerCountLimitForQueueKind uint16) ([]*Task, error) {
	...
	err := s.withTransaction(ctx, "get task from queue", func(cxt context.Context, tx *sqlx.Tx) error {
		...
		if workerCountLimitForQueueKind > 0 {
			...
			// по разным штатным причинах может быть больше задач в статусе "в работе", чем установленный на это лимит,
			// тогда вычисленная выше разница будет меньше нуля
			if limit <= 0 {
