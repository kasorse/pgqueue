...
func TestProcessor_e2e(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		mockMetricsCollector := NewMetricsCollectorMock(t).
		...
		qp := MakeSQLProcessor(sqlDB, mockMetricsCollector)
...
func (s *TestAppendTaskWithKeySuite) SetupSuite() {
	...
	s.taskKind = 1
	s.qp = MakeSQLProcessor(s.sqlDB, s.metricsCollector)
...
func (s *TestAppendTaskWithKeySuite) TestExternalKeySet() {
	...
	// act
	err := s.qp.AppendTask(ctx, s.taskKind, []byte("{}"), AppendTaskOptions{ExternalKey: expectedExternalKey})
...
func (s *TestAppendTaskWithKeySuite) TestDelay() {
	...
	// act
	err := s.qp.AppendTask(ctx, s.taskKind, []byte("{}"), AppendTaskOptions{
...
func (s *TestAppendTaskWithKeySuite) TestCancel() {
	...
	err1 := s.qp.AppendTask(ctx, s.taskKind, []byte("{}"), AppendTaskOptions{ExternalKey: keyToCancel, Delay: time.Second * 2})
	err2 := s.qp.AppendTask(ctx, s.taskKind, []byte("{}"), AppendTaskOptions{ExternalKey: keyToCancel, Delay: time.Second * 2})
	err3 := s.qp.AppendTask(ctx, s.taskKind, []byte("{}"), AppendTaskOptions{ExternalKey: keyToContinue, Delay: time.Second * 2})
	err4 := s.qp.AppendTask(ctx, s.taskKind, []byte("{}"), AppendTaskOptions{ExternalKey: keyToContinue, Delay: time.Second * 2})
	s.NoError(err1)
	s.NoError(err2)
	s.NoError(err3)
	s.NoError(err4)
...
func (s *TestAppendTaskWithKeySuite) TestEmptyExternalKey() {
	...
	// act
	err := s.qp.AppendTask(ctx, s.taskKind, []byte("{}"), AppendTaskOptions{ExternalKey: emptyKey})
