...
// MetricsCollector описывает интерфейс сборщика метрик.
type MetricsCollector interface {
	// IncBusyWorkers вызывается при получении воркером новой задачи.
	IncBusyWorkers(processorName string, taskKind int16)
	// DecBusyWorkers вызывается в самом конце обработки задачи.
	DecBusyWorkers(processorName string, taskKind int16)
	// MeasureTaskDuration вызывается после выполнения пользовательской
	// функции-обработчика задачи.
	MeasureTaskDuration(
		processorName string,
		taskKind int16,
		isSuccessful bool,
